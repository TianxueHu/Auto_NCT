...
colnames(dataframe)=c("melody", "index", "beat_pos","duration", "scale_deg","approaching","key")
...
dataframe<-read.csv('CTNCT.csv',header=TRUE, sep=",")[,2:16]

raw <- "raw_melody_num.krn"
count.fields(raw, sep = "\t")
rawfile<-read.delim2(raw, header=F, stringsAsFactors=F, sep = "", quote="\"")
rawfile<-rawfile[,c('V1','V2')]
colnames(rawfile)=c("melody", "index")
